...
const CONVERT_DEVICE_TOUCH = 7;
const CONVERT_DEVICE_GYRO = 8;
const CONVERT_FROM_FOV = 9;
...
function updateValue(coefficient){
	...
	var url = "https://simenatsu.github.io/beta?";
	for(args of url_args){
		url += args+"="+$("#"+args).val()+"&";
	}
	url += "round="+round;
	$("#copy_url").val(encodeURI(url));
	//$("#copy_url").val(encodeURI("http://simenatsu.github.io/beta?conversion_type="+$("#conversion_type").val()+"&"+"device_1="+$("#device_1").val()+"&"+"device_2="+$("#device_2").val()+"&"+"base_scope="+$("#base_scope").	val()+"&"+"base_sensitivity="+$("#base_sensitivity").val()+"&"+"tpp_camera="+$("#tpp_camera").val()+"&"+"fpp_camera="+$("#fpp_camera").val()+"&"+"1x="+$("#1x").val()+"&"+"2x="+$("#2x").val()+"&"+"3x="+$("#3x").val()+"&"+"4x="+$("#4x").val()+"&"+"6x="+$("#6x").val()+"&"+"8x="+$("#8x").val()));
}
...
function calculate(){
	...
	switch(conversion_type){
		case CONVERT_PPI:
			...
			break;
		case CONVERT_FROM_FOV:
			canVisible(DEVICE_SELECT);
			aspect_1 = device_1["resolution"]["height"]/device_1["resolution"]["width"];
			aspect_2 = device_2["resolution"]["height"]/device_2["resolution"]["width"];
			var aspect = Math.atan((aspect_2/aspect_1)*Math.tan(Math.PI/360))*(360/Math.PI);
			var coefficient = aspect; 
			break;
		case CONVERT_CAMERA:
			canVisible(DEVICE_SELECT);
		...
		case CONVERT_CONFORTABILITY_GYRO:
			...
			break;
		case CONVERT_DEVICE_TOUCH:
			canVisible(DEVICE_SELECT);
			camera_1 = device_1["dimensions"]["width"];
			camera_2 = device_2["dimensions"]["width"];
			var coefficient = camera_2/camera_1;
			break;
		case CONVERT_DEVICE_GYRO:
			canVisible(DEVICE_SELECT);
			camera_1 = device_1["dimensions"]["width"];
			camera_2 = device_2["dimensions"]["width"];
			var coefficient = camera_1/camera_2;
		...
		case 9:
			canVisible(DEVICE_SELECT);
			camera_1 = Math.sqrt(device_1["resolution"]["width"]**2+device_1["resolution"]["height"]**2);
			camera_2 = Math.sqrt(device_2["resolution"]["width"]**2+device_2["resolution"]["height"]**2);
			var coefficient = camera_1/camera_2;
			break;
		case 10:
			canVisible(DEVICE_SELECT);
		...
		case 11:
			canVisible(DEVICE_SELECT);
			camera_1 = device_1["resolution"]["height"];
			camera_2 = device_2["resolution"]["height"];
			var coefficient = camera_1/camera_2;
			break;
		case 12:
			canVisible(DEVICE_SELECT);
			camera_1 = device_1["resolution"]["width"];
			camera_2 = device_2["resolution"]["width"];
			var coefficient = camera_1/camera_2;
			break;
	}
